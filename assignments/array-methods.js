...
// Problem 1 Avarage Donation
let DonationAvarage = runners.reduce((total, item) => {
  let Avarage = item.donation + total;
  return Avarage;
}, 0);
console.log(DonationAvarage / 50);

// Problem 2 how many medium shirt ?
let runnersMediumSizeShirt = runners.filter(function(items) {
  return items.shirt_size === "M";
});
let Mnumber = runnersMediumSizeShirt.length;
console.log(Mnumber);

// Problem 3 WHO DONATED MORE THAN 200
let gigi = runners.filter(element => element.donation > 200);
console.log(gigi);
